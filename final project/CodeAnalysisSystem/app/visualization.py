...
def create_histogram(ax, function_lengths, file_name):
    ...
    plt.savefig(f'graphs/{file_name}_function_lengths_histogram.png')  # השתמש ב-plt.savefig
...
def create_pie_chart(ax, alert_types, file_name):
    ...
    plt.savefig(f'graphs/{file_name}_alerts_by_type_pie_chart.png')  # השתמש ב-plt.savefig
    # plt.close()  # סגור את התמונה
...
def create_graphs(alerts, function_lengths, file_name):
    ...
    plt.tight_layout()  # מתאימה את המרווחים בין הגרפים
    plt.show()  # הצגת כל הגרפים בחלון אחד

# הוספת פונקציה ליצירת תיקיית גרפים אם היא לא קיימת
